...
import librarysystem.libraryapi.controller.tool.SQLConnecter;
...
public class UserBookController {
    ...
    @RequestMapping("/book")
    public String userBook(Model model, HttpServletResponse response) {
        String sql = "SELECT * FROM book";
        List<Book> bookList = new ArrayList<>();
        new SQLConnecter(sql, response).getConnection(new SQLConnecter.CallBack() {
            @Override
            public void onConnected(ResultSet result) {
                try {
                    while (result.next()) {
                        Book book = new Book();
...
//        }
    ...
    public String studentBorrow(HttpServletRequest request, HttpServletResponse response) {
        ...
        final int borrowingCount, predetermineCount;
        String sql = "insert into borrow (user_id,bid) values (" + User.getInstance().id + "," + id + ");";
        new SQLConnecter(sql,response).getConnection(new SQLConnecter.CallBack() {
            @Override
            public void onConnected(ResultSet resultSet) {

            }
        });
        String sql2 = "select borrowing_count,predetermine_count from book where id = " + id + ";";
        new SQLConnecter(sql2,response).getConnection(new SQLConnecter.CallBack() {
            @Override
            public void onConnected(ResultSet result) {
                try {
                    while (result.next()) {
